...
describe("Add Partner Contact - Optimized", () => {
    beforeEach(() => {
        cy.visit("https://uat.mloflo.com/"); // Navigate to application URL
    });

    it("Adds a partner contact", () => {
        ...
        cy.get('.layout-grid-height-1 > .mileStone > .mileStone-tab > .nav-item:nth-child(2) > .nav-link')
          .should('be.visible')
          .click(); // Navigate to Partner Contacts tab
        
        cy.url().should("include", "/contacts#partner"); // Verify navigation to Partner Contacts section
        
        cy.get('.d-flex > .d-flex > .position-relative > .d-flex > .button')
          .should('be.visible')
          .click(); // Click on Add Partner Contact button
