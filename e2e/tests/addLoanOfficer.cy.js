...
describe("Add Loan Officer", () => {
    ...
    cy.get('.mb-sm > select').select('Loan Officer')
    // cy.get('#modalBody > div.form-row.mb-3.align-items-start > div.col-md-6.col-12.mb-sm > select').select("Loan Officer")

    const loName = faker.person.firstName()
    cy.get('#modalBody > :nth-child(2) > :nth-child(1) > .champ-form__cm-input').type(loName)
    ...
    cy.get('.content-box').should('contain', loName)
