...
describe("Add Loan Officer", () => {
    ...
    cy.get('.mb-sm > select').select('LOA')
    // cy.get('#modalBody > div.form-row.mb-3.align-items-start > div.col-md-6.col-12.mb-sm > select').select("Loan Officer")
   
    const loaName= faker.person.firstName()
    cy.get('#modalBody > :nth-child(2) > :nth-child(1) > .champ-form__cm-input').type(loaName)
    ...
    cy.get('.content-box').should('contain', loaName)
